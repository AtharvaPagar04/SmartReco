...
Learning paths use the V2 grounded planner: canonical questionnaire intent → domain-aware Mesh embedding queries and Qdrant candidates → SQL reload and deterministic pre-ranking → Mesh chat roadmap planning → deterministic composition, goal, budget, difficulty, and continuity validation → bounded Mesh repair → safe deterministic fallback. SQL remains authoritative for course truth and totals, and Mesh may select only supplied course IDs. Path-specific affinity distinguishes frontend, backend, full-stack, API, and UI/UX intent without changing the broad catalog categories. `LearningPathGenerationRun.metadata_json` records semantic retrieval, Mesh planning, repair, validation, and fallback provenance; the historical `used_mesh` field retains its prior semantic-retrieval meaning.
