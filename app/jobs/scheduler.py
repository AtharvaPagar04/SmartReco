...
def build_scheduler() -> AsyncIOScheduler:
    ...
    scheduler.add_job(process_email_deliveries, IntervalTrigger(seconds=60), id="recommendation-email-delivery", max_instances=1, coalesce=True, misfire_grace_time=120)
    scheduler.add_job(
        scan_session_followups,
        IntervalTrigger(seconds=settings.session_followup_scan_interval_seconds),
        id="session-followup-scan",
        max_instances=1,
        coalesce=True,
